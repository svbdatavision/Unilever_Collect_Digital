...
from Ecuador import (
    El_Rosado,Favorita, FARCOMED, DIFARE, Mega_Santa_Maria, FARMAENLACE
)
...
clientes_por_pais = {
    ...
    "Ecuador": [
        "Favorita", "FARCOMED", "DIFARE", "Mega Santa Maria", "Farmaenlace"
    ]
}

def main(page: ft.Page):
    page.title = "Procesador de Clientes"
    page.bgcolor = "#FFFFFF"
    ...
    def limpiar_campos():
        ...
        archivos_remittance = []
        archivo_fbl5n = None
        remittance_status.value = "Haz clic para adjuntar los Remittance"
        remittance_drop_area.bgcolor = "#FFFFFF"
        fbl5n_status.value = "Haz clic para adjuntar el FBL5N"
    ...
    def mostrar_procesamiento(pais):
        ...
        if not clientes:
            page.dialog = ft.AlertDialog(title=ft.Text("⚠️ No hay clientes disponibles para este país."))
    ...
    def procesar_cliente(e):
        ...
        if cliente not in clientes_solo_remittance and not archivo_fbl5n:
            page.dialog = ft.AlertDialog(title=ft.Text("⚠️ Este cliente requiere también el archivo FBL5N."))
            page.dialog.open = True
            page.update()
            return
    ...
    home_view = ft.Column(
        controls=[
            logo,
            ft.Text("Selecciona el país", size=20, color="#000000"),
            ft.Row([
                ft.ElevatedButton(
                    text=pais,
    ...
    processing_view = ft.Column(
        controls=[
            ...
            ft.Row([
                ft.ElevatedButton("Generar Template", on_click=procesar_cliente, bgcolor="#261E97", color="white"),
                ft.ElevatedButton("Descargar FBL5N", bgcolor="white", color="#137023",
                                  style=ft.ButtonStyle(
                                      shape=ft.RoundedRectangleBorder(radius=8),
